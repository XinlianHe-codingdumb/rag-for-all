...
export default function PrivacyPage() {
  return <LegalPage
    ...
    sections={[
      ...
      { title: "Where your data goes", paragraphs: ["Uploaded file bytes are stored in private object storage and document/run metadata is stored in the project database. Records are linked to the anonymous browser session on the server.", "When OpenAI-powered Embedding, Reranking, or Answer steps are used, the relevant text, question, and selected passages are sent to the configured OpenAI API. The optional no-RAG demonstration sends the question but deliberately does not send the uploaded document. Those requests are sent with API storage disabled. Local fallback steps do not send their text to OpenAI.", "Cloud infrastructure and model providers may process data in locations outside your country. Do not use this public beta for data that has regulatory, contractual, or residency requirements."] },
      { title: "Analytics and abuse protection", paragraphs: ["Product analytics measure anonymous visits, journey choices, section interest, step opens, and funnel completion so we can learn where the product is useful or confusing. We also count the type and outcome of model operations. Analytics do not include filenames, document text, questions, prompts, answers, email addresses, or raw IP addresses.", "For rate limiting, the service converts an IP address into a short one-way hash using a secret and the current date. The raw address is not stored by the application, and the hash changes each day."] },
